...
### 4b First run vs. future runs — `lib/resolve_lookup.py` (`class Lookup`)
...
- **How corrections are learned (no writeback):** the `lookup/` maps are a *projection of committed history*, so a corrected `category` is learned simply by being committed (`categorized/<year>.csv`) and re-projected by `build_lookup` on the next run. `category_override` is deliberately excluded from that projection. *(The commit step and* `build_lookup` *are not yet built; see §8.)*
...
- **M6 — Review approver UI** *(implemented)*: `schema.category_override` + `effective_category` + `CategorySource` enum (incl. `human-review`); `tools/review_approver/` (DearPyGui, mirrors `tools/rule_editor`) — two tabs (Review inbox / Everything else), select + **batch approve**, edit `corrected_description` / `category` (learned) / `category_override` (one-off), save in place. *Follow-up (not yet built): the commit step + `build_lookup` that close the learning loop (see §8).*
...
6. **Close the learning loop (commit + `build_lookup`)**: the maps are a *projection of committed history*, not a writeback — a corrected `category` is learned by being committed and re-projected next run. Two pieces are unbuilt: (a) the **commit step** appending approved review rows to `categorized/<year>.csv`; (b) `**build_lookup(history)`** in `resolve_lookup.py` that projects `norm_key(original) → category` (and `→ corrected_description`) from that history, which `resolve_batch` calls instead of loading static CSVs. `build_lookup` needs a conflict rule for a `norm_key` with differing categories across history (default: most-recent-commit wins). `category_override` is excluded from the projection. *(Row-identity is already resolved: the review file holds every row and is edited in place.)*
